...
### 🪄 站內已內建「貼網址自動帶入」（2026-09-24）

新增／編輯卡片的表單裡，「連結」欄位右邊的按鈕已擴充為**一次帶入標題＋圖片＋描述**。

按下去之後的處理順序：

| 順序 | 來源 | 說明 |
|---|---|---|
| 1 | YouTube oEmbed | 網址是 YouTube（含 Shorts / youtu.be）時走這條，最快且無次數限制 |
| 2 | Microlink API | 一般網站，實測 2.5 秒。免費 25 次/天 |
| 3 | r.jina.ai | 上一步失敗或額度用完時的後援（只有標題） |
| 4 | Wikipedia | 有標題但沒圖時，用標題查維基百科的圖片 |
...
- **標題與備註一律節錄**（2026-09-24 新增）：來源標題常夾帶帳號與站名（例：`帳號 (@handle) • Instagram reel`，實測 50 字）、簡介更常是整篇貼文原文（實測 IG Reels **1,159 字**）→ 直接倒進卡片會毀掉閱讀感
  - 標題上限 **30 字**、備註上限 **150 字**，超過就切在句讀處並補「…」（原文沒那麼長就原樣保留）
...
卡片上的備註**最多顯示 3 行**，超過就顯示「**展開全文 ▾**」，點一下展開、再點收合。

- **資料完全沒有被刪減**：只是顯示層收合，Firestore 裡仍是完整內容（判斷依據是「實際有沒有溢出」而不是字數，所以桌機多欄／手機單欄都準確）
- 收合後**藏不到 2 行**的就不收（展開鈕本身 44px，硬收反而讓卡片變高）
- 為什麼不直接砍資料：實測 34 張長備註幾乎都是整理過的重點（地址、價錢、注意事項、入口位置），砍掉就找不回來了 → 改為「顯示收合」兼顧閱讀節奏與資訊完整
- 展開鈕不會誤觸地圖：卡片點擊的委派處理已排除 `.note-toggle`
